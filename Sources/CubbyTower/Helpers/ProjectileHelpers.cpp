...
void CreateProjectile(entt::registry& registry, entt::entity tower, entt::entity enemy)
{
    ...
    const Position positionDiff = { enemyPosition.x - towerPosition.x, enemyPosition.y - towerPosition.y };
    const float distance = std::sqrt(positionDiff.x * positionDiff.x + positionDiff.y * positionDiff.y);
    if (distance == 0.0f) {
